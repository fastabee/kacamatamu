...
const tblStok = $('#tblStok').DataTable({
    processing: true,
    serverSide: true,
    ajax: { url: "<?= base_url('stok/kacamata/datatable') ?>", type: 'GET' },
    columns: [
...
$('#btnSimpanTransaksi').on('click', function () {
    ...
    $.ajax({
        url: "<?= base_url('stok/kacamata/transaksi') ?>", method: 'POST',
        data: { idkacamata: $('#txIdItem').val(), jenis: $('#txJenis').val(), jumlah, keterangan: $('#txKeterangan').val() },
