<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title fw-semibold mb-3">Stok Lensa</h5>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Transaksi Stok Lensa — <span id="lblNamaItem"></span></h5>
...
<div class="modal fade" id="modalRiwayat" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Riwayat Stok Lensa — <span id="lblNamaRiwayat"></span></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0">
                <table class="table table-sm table-striped mb-0">
                    <thead class="table-dark">
                        <tr>
                            <th>Tanggal</th><th>Jenis</th><th>Jumlah</th>
                            <th>Sebelum</th><th>Sesudah</th><th>Keterangan</th><th>Oleh</th>
                        </tr>
                    </thead>
...
const tblStok = $('#tblStok').DataTable({
    processing: true,
    serverSide: true,
    ajax: { url: "<?= base_url('stok/lensa/datatable') ?>", type: 'GET' },
    columns: [
...
$('#btnSimpanTransaksi').on('click', function () {
    ...
    if (!jumlah || jumlah < 1) { Swal.fire('Perhatian', 'Jumlah harus diisi dan lebih dari 0', 'warning'); return; }
    const $btn = $(this);
    $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Menyimpan...');
    $.ajax({
        url: "<?= base_url('stok/lensa/transaksi') ?>", method: 'POST',
        data: { idlensa: $('#txIdItem').val(), jenis: $('#txJenis').val(), jumlah, keterangan: $('#txKeterangan').val() },
...
$(document).on('click', '.btn-riwayat', function () {
    ...
    $.get("<?= base_url('stok/lensa/riwayat') ?>/" + iditem, function (res) {
        if (!res.data.length) {
